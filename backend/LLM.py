...
##helper function
def get_user_data(gender, age, history, occupation, diet):
    conn = sqlite3.connect(DATABASE_PATH)
    cursor = conn.cursor()

    query = """
    SELECT * FROM Users 
    WHERE WHERE id = 1 AND Gender = ? AND Age = ? AND FamilyMemberHistory = ? AND Occupation = ? AND Nutrition = ?
    """
    cursor.execute(query, (gender, age, history, occupation, diet))
